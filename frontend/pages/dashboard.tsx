    ...
    <div className="min-h-screen flex flex-col items-center justify-between bg-gradient-to-br from-green-100 via-blue-100 to-indigo-200 p-0">
      ...
      <header className="w-full sticky top-0 z-20 bg-white/90 backdrop-blur shadow-lg flex items-center justify-between px-8 py-5">
        <div className="flex items-center gap-3">
          ...
          <span className="text-3xl font-extrabold text-indigo-700 tracking-tight drop-shadow">EcoWaste</span>
        </div>
        <nav className="flex gap-8 items-center">
          <button onClick={handleGoHome} className="text-blue-600 hover:text-blue-800 font-semibold transition-colors text-lg">Home</button>
          <span className="text-gray-300 text-xl">|</span>
          <span className="text-indigo-700 font-bold text-lg">Dashboard</span>
          <span className="text-gray-300 text-xl">|</span>
          <button onClick={() => { localStorage.clear(); router.push("/"); }} className="text-red-500 hover:text-red-700 font-semibold transition-colors text-lg">Logout</button>
        </nav>
      </header>
      ...
      {showPastOrders && (
        <div className="mt-4 w-full max-w-2xl animate-fade-in">
          <div className="bg-white/90 rounded-3xl shadow-2xl p-10 transition-shadow hover:shadow-3xl">
            <h2 className="text-3xl font-extrabold mb-8 text-indigo-700 border-b-2 border-indigo-100 pb-3 tracking-tight">Past Orders</h2>
            {orders.length === 0 ? (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:gap-10 gap-3">
                      <div className="flex-1">
                        <span className="font-semibold text-gray-700">Item:</span> {order.item_type}
                      </div>
                      <div className="flex-1">
                        <span className="font-semibold text-gray-700">Quantity:</span> {order.quantity} Kgs
                      </div>
                    </div>
                    <div className="flex flex-col md:flex-row md:gap-10 gap-3 mt-2">
                      <div className="flex-1">
                        <span className="font-semibold text-gray-700">Pickup Date:</span> {order.pickup_date}
                      </div>
                      <div className="flex-1">
                        <span className="font-semibold text-gray-700">Order Date:</span> {order.order_date}
                      </div>
                    </div>
                  </li>
                ))}
      ...
      {showCreateOrder && (
        ...
        >
          <div className="bg-white/90 rounded-3xl shadow-2xl p-10 transition-shadow hover:shadow-3xl">
            <h2 className="text-3xl font-extrabold mb-8 text-indigo-700 border-b-2 border-indigo-100 pb-3 tracking-tight">Create Order</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div>
                ...
                >
                  ...
                  {selectedCategory &&
                    (itemsData.find((cat: any) => cat.category === selectedCategory)?.items || []).map((item: string) => (
                      <option key={item} value={item}>{item}</option>
                    ))}
                </select>
              </div>
              <div>
                <label className="block mb-2 font-semibold text-lg">Quantity (Kgs)</label>
                <input
                  type="number"
                  min={0.01}
                  step={0.01}
            ...
            {Object.keys(selectedItems).length > 0 && (
              <div className="mb-6">
                <h3 className="font-semibold mb-3 text-indigo-700 text-lg">Items in Order</h3>
                <ul className="list-disc list-inside text-gray-700 text-lg">
                  {Object.entries(selectedItems).map(([item, qty]) => (
                    <li key={item}>{item}: {qty} Kgs</li>
                  ))}
                </ul>
              </div>
            )}
            <div className="mb-6">
              <label className="block mb-2 font-semibold text-lg">Pickup Date</label>
              <input
                type="datetime-local"
                value={pickupDate}
      ...
      <div className="w-full max-w-3xl bg-gradient-to-r from-green-50 via-blue-50 to-indigo-100 rounded-3xl shadow-2xl p-10 mt-20 mb-10 flex flex-col items-center animate-fade-in border border-indigo-100">
        <div className="flex items-center gap-5 mb-4">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-indigo-800 drop-shadow">Why is Waste Management Necessary?</h1>
        </div>
        ...
        <blockquote className="italic text-indigo-600 mt-6 text-center text-lg md:text-xl">“The greatest threat to our planet is the belief that someone else will save it.”</blockquote>
      </div>
      ...
      <footer className="w-full mt-8 py-8 bg-white/90 backdrop-blur border-t border-indigo-100 flex flex-col items-center text-gray-500 text-base">
        ...
        <div className="mt-2 italic text-green-700">Together, we build a cleaner future.</div>
      </footer>
    </div>
  );
}
